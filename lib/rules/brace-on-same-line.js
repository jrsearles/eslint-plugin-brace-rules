...
var factory = function(context) {
    ...
    function insertBreakBefore(token) {
            ...
            _.forEach(blockProperties, function(blockProp) {
                ...
                if (startSameLine !== options[type]) {
                    context.report({
                        ...
                        fix: startSameLine ? insertBreakBefore(curlyToken) : removeBreakBetween(previousToken, curlyToken)
                    });
                }
                ...
                if (curlyToken.loc.start.line === block.body[0].loc.start.line) {
                    context.report({
                        ...
                        fix: insertBreakBefore(curlyToken)
                    })
                }
    ...
    function checkSwitchStatement(node) {
        ...
        if (options.SwitchStatement !== sameLine) {
            context.report({
                ...
                fix: sameLine ? insertBreakBefore(tokens[1]) : removeBreakBetween(tokens[0], tokens[1])
            });
        }
    }
